...
public class ErrorSquarePID implements IController{
    ...
    public void initialize(double... params) {
        if(params.length < 8) {
            throw new IllegalArgumentException("ErrorSquarePID requires 8 parameters: setpoint, direction, min, max, PC, kp, ki and kd");
        }
    ...
    public double update(double... input) {
        ...
        double output = Math.abs(error) * (proportional + integral + differentiator);
